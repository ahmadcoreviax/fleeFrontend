...
export default function CarDetailsPage() {
  ...
  return (
    <>
      <Header />
      <div className="min-h-screen bg-[#0e1111] mt-20 text-white p-4 md:p-8 max-w-7xl mx-auto">
        {/* Images Section */}
        <motion.div
        ...
        >
          ...
          <div className="bg-[#1a1d1d] rounded-2xl p-6 shadow-xl border border-[#e81828]/30">
            ...
            {car?.discountedPercentage && (
              <div className="mt-4">
                <span className="inline-block bg-gradient-to-r from-red-600 to-red-400 text-white text-sm font-bold px-4 py-1 rounded-full shadow-lg animate-pulse">
                  🔥 {car.discountedPercentage}% OFF SALE
                </span>
              </div>
            )}
            {/* Charges with Discount Calculation */}
            <div className="flex flex-wrap gap-4 mt-4">
                ...
                {car?.discountedPercentage ? (
                  <span>
                    <span className="line-through mr-1 text-gray-400">
                      {car.perDayCharges}/day
                    </span>
                    <span>
                      {Math.round(
                        car?.perDayCharges -
                          (car?.perDayCharges * car?.discountedPercentage) / 100
                      )}
                      /day
                    </span>
                  </span>
                ) : (
                  <span>{car.perDayCharges}/day</span>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 text-white">
                ...
                {car?.discountedPercentage ? (
                  <span>
                    <span className="line-through mr-1 text-gray-400">
                      {car?.perWeekCharges}/week
                    </span>
                    <span>
                      {Math.round(
                        car?.perWeekCharges -
                          (car?.perWeekCharges * car?.discountedPercentage) /
                            100
                      )}
                      /week
                    </span>
                  ...
                  <span>{car?.perWeekCharges}/week</span>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 text-gray-300">
                ...
                {car?.discountedPercentage ? (
                  <span>
                    <span className="line-through mr-1 text-gray-400">
                      {car?.perMonthCharges}/month
                    </span>
                    <span>
                      {Math.round(
                        car?.perMonthCharges -
                          (car?.perMonthCharges * car?.discountedPercentage) /
                            100
                      )}
                      /month
                    </span>
                  ...
                  <span>{car?.perMonthCharges}/month</span>
                )}
              </div>
            </div>
